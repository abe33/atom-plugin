...
function distinctID() {
  console.log("getting distinctID");
  var id = localconfig.get('distinctID');
  if (id === undefined) {
    // use the atom level UID
    console.log("falling back");
    id = atom.config.get('exception-reporting.userId');
    if (id === undefined) {
      // fall back to generating and storing out own UID
      console.log("falling back");
      id = crypto.randomBytes(32).toString('hex');
    }
    console.log("setting distinctID: "+id);
